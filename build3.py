...
if __name__ == '__main__':
    df = pd.read_csv(f"TA_PersonalCodigotrabajo.csv", low_memory=False,sep=";",encoding="latin",usecols=PersonalPlantaDICT)
    df = addColumns(df)
    for i in df["organismo_nombre"].unique():
        organismo = df[df["organismo_nombre"] == i]
        organismo.to_excel(f"build3/{i}.xlsx", index=False)
